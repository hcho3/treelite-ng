...
void identity(treelite::Model const&, std::uint32_t, InputT*) {}

template <typename InputT>
void signed_square(treelite::Model const&, std::uint32_t, InputT* elem) {
  InputT const margin = *elem;
  *elem = std::copysign(margin * margin, margin);
}

template <typename InputT>
void hinge(treelite::Model const&, std::uint32_t, InputT* elem) {
  *elem = (*elem > 0 ? InputT(1) : InputT(0));
}

template <typename InputT>
void sigmoid(treelite::Model const& model, std::uint32_t, InputT* elem) {
  InputT const val = *elem;
  *elem = InputT(1) / (InputT(1) + std::exp(-model.sigmoid_alpha * val));
}

template <typename InputT>
void exponential(treelite::Model const&, std::uint32_t, InputT* elem) {
  *elem = std::exp(*elem);
}

template <typename InputT>
void exponential_standard_ratio(treelite::Model const& model, std::uint32_t, InputT* elem) {
  *elem = std::exp2(-*elem / model.ratio_c);
}

template <typename InputT>
void logarithm_one_plus_exp(treelite::Model const&, std::uint32_t, InputT* elem) {
  *elem = std::log1p(std::exp(*elem));
}

template <typename InputT>
void identity_multiclass(treelite::Model const&, std::uint32_t, InputT*) {}

template <typename InputT>
void softmax(treelite::Model const&, std::uint32_t num_class, InputT* row) {
  float max_margin = row[0];
  double norm_const = 0.0;
  float t;
  for (std::uint32_t i = 1; i < num_class; ++i) {
    if (row[i] > max_margin) {
      max_margin = row[i];
    }
  }
  for (std::uint32_t i = 0; i < num_class; ++i) {
...
template <typename InputT>
void multiclass_ova(treelite::Model const& model, std::uint32_t num_class, InputT* row) {
  for (std::uint32_t i = 0; i < num_class; ++i) {
